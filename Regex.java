...
public enum Regex {
    END("end\\s*"),
//##################### user creating #####################
    CREATE_NEW_USER("user\\s+create\\s+-u (?<username>[\\w\\s]+) -p (?<password>[\\w\\s@#$%&*]+) (?<passwordConfirmation>[\\w\\s@#$%&*]+) –email (?<email>[\\w\\s@\\.]+) -n (?<nickname>[\\w\\s]+)"),
    CREATE_NEW_USER_WITH_RANDOM_PASSWORD("user\\s+create\\s+-u (?<username>[\\w\\s]+) -p random –email (?<email>[\\w\\s@\\.]+) -n (?<nickname>[\\w\\s]+)"),
...
//##################### admin menu #####################
    ...
    ADD_CARD("add new card (?<id>\\d+) (?<name>\\w+) (?<defence>\\d+) (?<duration>\\d+) (?<damage>\\d+) (?<upgradeLevel>\\d+) (?<upgradeCost>\\d+)\\s*"),

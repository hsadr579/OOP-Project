...
public class DB {
    ...
    public static String[] getUserData(String username) {
        ...
        try {
            Statement stmt = connection.createStatement();
            ResultSet rs = stmt.executeQuery(sql);
            return new String[] {rs.getString("username"), rs.getString("password"), rs.getString("security_question_id"), rs.getString("security_question_answer"), rs.getString("email"), rs.getString("nickname"), rs.getString("xp"), rs.getString("coins"), rs.getString("clan_id"), rs.getString("last_failed_login"), rs.getString("login_fail_number")};
        } catch (SQLException e) {
            System.out.println(e.getMessage());
            return null;
        }
